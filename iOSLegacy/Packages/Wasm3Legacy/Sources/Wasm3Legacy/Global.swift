...
public class Global<T: WasmType> {
    ...
    func set(_ value: T) throws {
        ...
        // swiftlint:disable force_cast
        let valueUnion: M3ValueUnion
        switch T.self {
            case is Int32.Type:
                valueUnion = M3ValueUnion(i32: UInt32(safe: value as! Int32))
            case is Int64.Type:
                valueUnion = M3ValueUnion(i64: UInt64(safe: value as! Int64))
            case is Float32.Type:
                valueUnion = M3ValueUnion(f32: value as! Float32)
            case is Float64.Type:
                valueUnion = M3ValueUnion(f64: value as! Float64)
            case is Int.Type:
                valueUnion = M3ValueUnion(i64: UInt64(safe: Int64(value as! Int)))
            case is UInt32.Type:
                valueUnion = M3ValueUnion(i32: value as! UInt32)
            case is UInt64.Type:
                valueUnion = M3ValueUnion(i64: value as! UInt64)
            default:
